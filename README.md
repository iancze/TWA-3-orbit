...
This repository accompanies the publication [A coplanar circumbinary protoplanetary disk in the TWA 3 triple M dwarf system](https://arxiv.org/abs/2102.11875) by Ian Czekala, Álvaro Ribas, Nicolás Cuello, Eugene Chiang, Enrique Macías, Gaspard Duchêne, Sean M. Andrews, and Catherine C. Espaillat.

This repository contains the datasets and pymc3 models, the most relevant being the "joint" fits. The MCMC samples themselves for the orbital fits appearing in the publication are hosted at a Zenodo repository [here](https://zenodo.org/record/4568830#.YDvNtF1KidY).

## Installation 

### Pip

It helps to install this locally to an environment. In my case, I did
...
8. Same, but now including disk orbit normal (evaluated as multi-dimenisonal Gaussian  from dynamical modeling and directly calculating mutual inclinations for all angles. 
...
### i_disk > 90 or < 90
In theory, we should be able to tell from the sub-mm emission alone which side of the disk is near. However, the CO emission is sufficiently faint and with the Nuker profile I'm not sure we can do this without error. If I had to guess from the emission alone, it seems like the Southern lobe is the far side, which implies i_disk < 90. This would be striking because then the disk and the outer binary are retrograde.
...
We are reporting fits for both i_disk < 90 and i_disk > 90. We might also want to report fits for the two modes of Omega_B. 
