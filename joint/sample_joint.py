...
with pm.Model() as model:
    ...
    logP_inner = pm.Uniform(
        "logP_inner", lower=0, upper=np.log(50.0), testval=np.log(34.87846)
    )  # days
    ...
    cos_incl_inner = pm.Uniform("cos_incl_inner", lower=-1.0, upper=1.0, testval=-0.724)
    ...
    t_periastron_inner = pm.Uniform(
        "t_periastron_inner", lower=4020.0, upper=4070.0, testval=4039.14
    )  # + 2400000 + jd0
    ...
    gamma_outer = pm.Uniform(
        "gamma_outer", lower=0, upper=15, testval=9.8
    ...
    logjit_keck = pm.Uniform(
        "logjitterkeck", lower=-5.0, upper=np.log(10), testval=-0.27
    )
    logjit_feros = pm.Uniform(
        "logjitterferos", lower=-5.0, upper=np.log(10), testval=1.32
    )
    logjit_dupont = pm.Uniform(
        "logjitterdupont", lower=-5.0, upper=np.log(10), testval=0.66
    )
...
with model:
    ...
    trace = pm.sample(
        draws=4000, tune=2000, step_kwargs=dict(target_accept=0.9), chains=4
    )
