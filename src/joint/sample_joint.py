...
with pm.Model() as model:
    ...
    logP_inner = pm.Uniform(
        "logP_inner", lower=0, upper=np.log(50.0), testval=np.log(34.879)
    )  # days
    ...
    cos_incl_inner = pm.Uniform("cos_incl_inner", lower=-1.0, upper=1.0, testval=0.67)
    ...
    t_periastron_inner = pm.Uniform(
        "t_periastron_inner", lower=4050.0, upper=4100.0, testval=4073.0
    )  # + 2400000 + jd0
    ...
    cos_incl_outer = pm.Uniform("cos_incl_outer", lower=-1.0, upper=1.0, testval=-0.60)
    ...
    gamma_outer = pm.Uniform(
        "gamma_outer", lower=0, upper=15, testval=10.0
    ...
    MB = PosNormal("MB", mu=0.3, sd=0.3, testval=0.5)
    ...
    offset_keck = pm.Normal("offsetKeck", mu=-1.6, sd=5.0)  # km/s
    offset_feros = pm.Normal("offsetFeros", mu=1.8, sd=5.0)  # km/s
    ...
    rv3_keck = (
        conv * orbit_outer.get_planet_velocity(keck3[0])[2] + gamma_outer + offset_keck
    )
    ...
    logjit_cfa = pm.Uniform("logjittercfa", lower=-5.0, upper=np.log(4), testval=1.18)
    logjit_keck = pm.Uniform(
        "logjitterkeck", lower=-5.0, upper=np.log(4), testval=-0.07
    )
    logjit_feros = pm.Uniform(
        "logjitterferos", lower=-5.0, upper=np.log(4), testval=1.32
    )
    logjit_dupont = pm.Uniform(
        "logjitterdupont", lower=-5.0, upper=np.log(4), testval=0.66
    )
    ...
    pm.Normal(
        "keckRV3Obs", mu=rv3_keck, observed=keck3[1], sd=get_err(keck3[2], logjit_keck)
    )
    ...
    theta_diff_inner = tt.arctan2(
        tt.sin(theta_inner - anthonioz[3]), tt.cos(theta_inner - anthonioz[3])
    )
    ...
    theta_diff_outer = tt.arctan2(
        tt.sin(theta_outer - wds[3]), tt.cos(theta_outer - wds[3])
    )
...
with model:
    trace = pm.sample(
        draws=2000, tune=1000, step_kwargs=dict(target_accept=0.9), chains=4
    )

# save as CSV file
df = pm.trace_to_dataframe(trace)
df.to_csv(f"{outdir}current.csv")
...
pm.summary(trace)
az.plot_trace(trace)  # , var_names=vars)
plt.savefig(f"{outdir}traceplot.png", dpi=120)
