...
with pm.Model() as model:
    ...
    cos_incl = pm.Uniform(
        "cosIncl", lower=-1.0, upper=1.0, testval=np.cos(140.0 * deg)
    ...
    rho_model, theta_model = orbit.get_relative_angles(d.wds[0], parallax)  # arcsec

    # evaluate the astrometric likelihood functions
    pm.Normal("obs_rho", mu=rho_model, observed=d.wds[1], sd=d.wds[2])
    theta_diff = tt.arctan2(
        tt.sin(theta_model - d.wds[3]), tt.cos(theta_model - d.wds[3])
    )  # wrap-safe
    pm.Normal("obs_theta", mu=theta_diff, observed=0.0, sd=d.wds[4])
